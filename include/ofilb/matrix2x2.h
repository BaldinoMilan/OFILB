...
namespace oi {
	...
	public:
		...
		friend tmat2<type> operator+ (const tmat2<type>& m, type t);
		template<typename T>
		friend tmat2<T> operator+ (const T& t, tmat2<T> m);
		template<typename type>
		friend tmat2<type> operator+ (const tmat2<type>& m, tmat2<type> o);
	
		template<typename T>
		friend tmat2<T> operator- (const tmat2<T>& m, T t);
		template<typename T>
		friend tmat2<T> operator- (const T& t, tmat2<T> m);
		template<typename T>
		friend tmat2<T> operator- (const tmat2<T>& m, tmat2<T> o);

		template<typename T>
		friend tmat2<T> operator* (const tmat2<T>& m, T t);
		template<typename T>
		friend tmat2<T> operator* (const T& t, const tmat2<T> m);
		template<typename T>
		friend tmat2<T> operator* (const tmat2<T>& m, tmat2<T> o);
